...
function CartModal({ onClose, onCheckout }) {
  ...
  const handleItemCart = (product, action) => {
    switch (action) {
      case "add":
                  ...
                  <div className="flex items-center space-x-3 border-b border-dashed border-gray-200 py-4" key={product?.id}>
                    ...
                    <div className="w-full">
                      <h6 className="relative mb-px line-clamp-2 pr-8 text-sm font-bold text-gray-800 hover:line-clamp-none">
                        {product?.title}
                        <span className="absolute right-0 top-0 cursor-pointer" role="presentation" onClick={() => handleItemCart(product, "remove")}>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center overflow-hidden rounded-full border border-gray-300">
                          <button type="button" aria-label="Minus Item" className="px-3 py-2 leading-normal" onClick={() => handleItemCart(product, "minus")}>
                            <MinusIcon size={14} strokeWidth={3} />
                          </button>
                          <p className="text-sm">{product?.quantity}</p>
                          <button type="button" aria-label="Add Item" className="px-3 py-1.5 leading-normal" onClick={() => handleItemCart(product, "add")}>
